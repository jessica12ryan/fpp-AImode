 ...
 */
$pluginDir = __DIR__;
// GUIDELINE §1.1 — resolve log dir via $settings['logDirectory'], not hard-coded path
$logDir = $settings['logDirectory'] ?? $GLOBALS['settings']['logDirectory'] ?? getenv('LOGDIR') ?: null;
if (!$logDir) {
    $media = $settings['mediaDirectory'] ?? $GLOBALS['settings']['mediaDirectory'] ?? getenv('MEDIADIR') ?: null;
    $logDir = $media ? rtrim($media, '/') . '/logs' : null;
}
$logFile = $logDir ? rtrim($logDir, '/') . '/plugin-fpp-AImode.log' : 'plugin-fpp-AImode.log (logDirectory not configured)';
    ...
    <fieldset class="border p-3">
        ...
        <div class="p-3">
            <p>
                ...
                &nbsp;&nbsp;
                <input type="button" class="buttons" value="↻ Refresh" onclick="aimLogs.refresh();">
                <span class="text-secondary" style="font-size:12px; margin-left:8px;">Keys are redacted to ***. Check FPPD logs for deeper errors.</span>
            </p>
...
var aimLogs = {
    refresh: function() {
        ...
        $.ajax({
            url: 'api/plugin/fpp-AImode/logs',
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                if (data.success && data.entries) {
                    ...
                    $('#log_body').html('<tr><td colspan="4" style="text-align:center;padding:20px;" class="text-danger">Error loading logs: ' + (data.error || 'Unknown error') + '</td></tr>');
                }
            },
            error: function(xhr) {
                ...
                $('#log_body').html('<tr><td colspan="4" style="text-align:center;padding:20px;" class="text-danger">' + msg + '</td></tr>');
            }
        });
    }
};
function escHtml(s) { if (s == null) return ''; return String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;'); }
$(document).ready(function() { aimLogs.refresh(); });
</script>
